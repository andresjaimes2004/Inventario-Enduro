...
//         {/* Header */}
//         <header className="w-full md:w-[240px] h-[80px] md:min-h-screen bg-[#FF7700] flex p-4 md:p-0">
  ...
  return (
    <>
      <div className="flex flex-col md:flex-row h-screen">
        {/* Header - Dashboard Fijo */}
        <header className="w-full md:w-[240px] h-[80px] md:h-full bg-[#FF7700] flex p-4 md:p-0 flex-shrink-0">
          <div className="w-full flex md:flex-col items-center md:items-center justify-between md:justify-start md:gap-y-20">
            <img
              src="src/assets/Logo.png"
              alt="Logo Enduro"
            ...
            <div className="flex md:w-full md:items-center">
              ...
              <ul
                ...
                transition-all duration-300 ease-in-out z-50 ${
                  isOpen ? "h-[125px]" : "h-[0px]"
                } md:h-auto overflow-hidden md:overflow-visible`}
              >
                {menuItems.map((item) => (
                  <li key={item.label} className="w-full">
                    <Link
                      ...
                      onClick={() => setIsOpen(false)} // Cierra el menú móvil al hacer clic
                    >
                      {item.icon}
                      <span className="flex-1">{item.label}</span>
                    </Link>
        ...
        <main className="flex-1 bg-[#e6e6e6] overflow-y-auto">
          <Routes>
            <Route path="/" element={<Navigate to="/Inicio" replace />} />
            <Route path="/Inicio" element={<Inicio />} />
            <Route path="/Productos" element={<Productos />} />
